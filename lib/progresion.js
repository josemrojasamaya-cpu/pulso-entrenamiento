...
function sugerirPeso(historial, repObjetivo, equipo, intensidadMax = 1.0) {
    ...
    // Se toma la mejor serie reciente, no la última: la última puede ser
    // una serie de descarga o una que se cortó, y no representa lo que
    ...
    for (const s of recientes) {
        ...
        if (e && (!mejor || e.valor > mejor.rm)) {
            mejor = { rm: e.valor, confianza: e.confianza, serie: s };
        }
    }
    ...
    const peso = redondear(conTope, equipo);
...
module.exports = {
    estimar1RM, pesoPara, redondear, factorPorEsfuerzo,
