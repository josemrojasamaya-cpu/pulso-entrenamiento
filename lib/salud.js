 ...
 *   evitarSupino   descarta lo que se hace acostado boca arriba
...
const CONDICIONES = {
    ...
    embarazo: {
        ...
        evitarPatrones: [],
        sinImpacto: true,
        // El aviso de abajo afirma que se quitan los ejercicios boca
        // arriba. Antes eso era falso: la afirmación estaba en el texto
        // pero no en ninguna regla, y el sistema seguía proponiendo dead
        // bug, puente de glúteos y press francés.
        evitarSupino: true,
        avisos: [
...
/**
 * Cuánto endurece cada nivel de severidad los límites de su condición.
 *
 * La severidad se venía guardando y devolviendo, pero ninguna regla la
 * leía: una hernia discal declarada como *alta* recibía exactamente el
 * mismo trato que una molestia leve. Si se pregunta, tiene que servir
 * para algo.
 */
const POR_SEVERIDAD = {
    leve:     { intensidad: 1.10, exigencia: +1, descanso: 0.9 },
    moderada: { intensidad: 1.00, exigencia:  0, descanso: 1.0 },
    alta:     { intensidad: 0.85, exigencia: -1, descanso: 1.2 }
};
 ...
 *
 * @param {Array<string|{codigo:string,severidad?:string}>} codigos
 *        condiciones activas. Acepta códigos sueltos o el objeto con su
 *        severidad; sin severidad se asume moderada.
 */
function restricciones(codigos = []) {
    const activas = codigos
        .map(entrada => {
            const codigo = typeof entrada === "string" ? entrada : (entrada && entrada.codigo);
            const base = CONDICIONES[codigo];
            if (!base) return null;

            const sev = (typeof entrada === "object" && entrada && entrada.severidad) || "moderada";
            const ajuste = POR_SEVERIDAD[sev] || POR_SEVERIDAD.moderada;

            return {
                ...base,
                codigo,
                severidad: sev,
                // Una condición leve afloja un poco; una alta aprieta. Nunca
                // por encima de 1: el tope sigue siendo un tope.
                intensidadMax: Math.min(1, base.intensidadMax * ajuste.intensidad),
                exigenciaMax: Math.max(1, Math.min(5, base.exigenciaMax + ajuste.exigencia)),
                descansoMin: Math.round(base.descansoMin * ajuste.descanso)
            };
        })
        .filter(Boolean);
    ...
    const base = {
        ...
        evitarSupino: false,
        avisos: [],
        condiciones: activas.map(a => ({
            codigo: a.codigo, nombre: a.nombre, severidad: a.severidad
        }))
    };
    ...
    for (const c of activas) {
        ...
        base.evitarSupino = base.evitarSupino || Boolean(c.evitarSupino);
...
function esApto(ejercicio, codigosCondicion, limites) {
    ...
    // El impacto se comprueba ACÁ, junto con el resto de los filtros.
    // Antes se calculaba en `restricciones()` y sólo se consultaba dentro
    // del bloque de cardio, con una expresión regular sobre el nombre del
    // ejercicio: siete de las once condiciones lo exigían y ninguna lo
    // conseguía de verdad.
    if (limites.sinImpacto && ejercicio.impacto) {
        return { apto: false, motivo: "es de impacto, y tus condiciones lo desaconsejan" };
    }

    if (limites.evitarSupino && ejercicio.supino) {
        return { apto: false, motivo: "se hace boca arriba, y eso está desaconsejado en tu caso" };
    }
