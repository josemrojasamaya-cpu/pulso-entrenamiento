 ...
 */
function generar({ perfil, condiciones = [], catalogo, fecha, usuarioId,
                   historial = {}, sesionIndice = 0, ejerciciosRecientes = [] }) {

    const limites = restricciones(condiciones);
    ...
    for (const ranura of plantilla.ranuras) {
        ...
        if (medida === "segundos") {
            ...
            min = Math.round(20 * escala);
            max = Math.round(35 * escala);
            peso = null;
        ...
        elegidos.push({
            ...
            rep_max: max,
            medida,
            descanso_seg: ranura.principal ? descanso : Math.max(45, Math.round(descanso * 0.7)),
